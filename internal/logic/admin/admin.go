package admin

import (
	"context"
	"goframe_shop/internal/dao"
	"goframe_shop/internal/model"
	"goframe_shop/internal/model/entity"
	"goframe_shop/internal/service"
	"goframe_shop/utility"

	"github.com/gogf/gf/v2/database/gdb"
	"github.com/gogf/gf/v2/encoding/ghtml"
	"github.com/gogf/gf/v2/util/grand"

	"github.com/gogf/gf/v2/frame/g"
)

type sAdmin struct{}

func init() {
	service.RegisterAdmin(New())
}

func New() *sAdmin {
	return &sAdmin{}
}

func (s *sAdmin) Create(ctx context.Context, in model.AdminCreateInput) (model.AdminCreateOutput, error) {
	var out model.AdminCreateOutput
	// 不允许HTML代码
	if err := ghtml.SpecialCharsMapOrStruct(in); err != nil {
		return out, err
	}
	//处理加密盐和密码的逻辑
	UserSalt := grand.S(10)
	in.Password = utility.EncryptPassword(in.Password, UserSalt)
	in.UserSalt = UserSalt
	//插入数据返回id
	lastInsertID, err := dao.AdminRepo.Ctx(ctx).Data(in).InsertAndGetId()
	if err != nil {
		return model.AdminCreateOutput{}, err
	}
	return model.AdminCreateOutput{AdminId: int(lastInsertID)}, err
}

func (s *sAdmin) GetUserByUserNamePassword(ctx context.Context, in model.UserLoginInput) map[string]interface{} {
	//验证账号密码是否正确
	AdminRepo := entity.Admin{}
	err := dao.AdminRepo.Ctx(ctx).Where(dao.AdminRepo.Columns().Name, in.Name).Scan(&AdminRepo)
	if err != nil {
		return nil
	}
	if utility.EncryptPassword(in.Password, AdminRepo.UserSalt) != AdminRepo.Password {
		return nil
	} else {
		return g.Map{
			"id":       AdminRepo.Id,
			"username": AdminRepo.Name,
		}
	}
}

func (s *sAdmin) Delete(ctx context.Context, id uint) error {
	return dao.AdminRepo.Transaction(ctx, func(ctx context.Context, tx gdb.TX) error {
		// 删除内容
		_, err := dao.AdminRepo.Ctx(ctx).Where(g.Map{
			dao.AdminRepo.Columns().Id: id,
		}).Delete()
		return err
	})
}

func (s *sAdmin) Update(ctx context.Context, in model.AdminUpdateInput) error {
	//判断是否修改了密码
	if in.Password != "" {
		UserSalt := grand.S(10)
		in.Password = utility.EncryptPassword(in.Password, UserSalt)
		in.UserSalt = UserSalt
	}
	//更新操作
	_, err := dao.AdminRepo.
		Ctx(ctx).
		Data(in).
		OmitEmpty(). //注意：使用OmitEmpty()实现部分更新
		Where(dao.AdminRepo.Columns().Id, in.Id).
		Update()
	return err
}

//1.获得*gdb.Model对象，方面后续调用
//2. 实例化响应结构体
//3. 分页查询
//4. 再查询count，判断有无数据
//5. 延迟初始化list切片 确定有数据，再按期望大小初始化切片容量
//6. 把查询到的结果赋值到响应结构体中

// List 查询内容列表
func (s *sAdmin) List(ctx context.Context, in model.AdminGetListInput) (*model.AdminGetListOutput, error) {
	//1.获得*gdb.Model对象，方便后续调用
	m := dao.AdminRepo.Ctx(ctx)
	//2. 实例化响应结构体
	out := &model.AdminGetListOutput{
		Page: in.Page,
		Size: in.Size,
	}
	//3. 分页查询
	listModel := m.Page(in.Page, in.Size)
	var err error
	//4. 再查询count，判断有无数据
	out.Total, err = m.Count()
	if err != nil || out.Total == 0 {
		//解决空数据返回[] 而不是返回nil null的问题
		out.List = make([]model.AdminGetListOutputItem, 0, 0)
		return out, err
	}
	//5. 延迟初始化list切片 确定有数据，再按期望大小初始化切片容量
	out.List = make([]model.AdminGetListOutputItem, 0, out.Total)
	//6. 把查询到的结果赋值到响应结构体中
	if err = listModel.Scan(&out.List); err != nil {
		return out, err
	}
	return out, nil
}

func (s *sAdmin) GetAdminByNamePassword(ctx context.Context, in model.UserLoginInput) map[string]interface{} {
	//验证账号密码是否正确
	AdminRepo := entity.Admin{}
	err := dao.AdminRepo.Ctx(ctx).Where("name", in.Name).Scan(&AdminRepo)
	if err != nil {
		return nil
	}
	if utility.EncryptPassword(in.Password, AdminRepo.UserSalt) != AdminRepo.Password {
		return nil
	} else {
		return g.Map{
			"id":       AdminRepo.Id,
			"username": AdminRepo.Name,
		}
	}
}
